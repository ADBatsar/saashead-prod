...
import { TrendingUp, FolderKanban, BarChart3, Download, Filter, CalendarDays, Briefcase } from "lucide-react";
...
export default function ReportsPage() {
  ...
  return (
    ...
    >
      {/* View Toggle Buttons */}
      <div className="flex bg-white border border-slate-200 shadow-sm rounded-xl p-1 overflow-x-auto mb-6 w-max">
        <button onClick={() => setView("monthly")} className={`px-4 py-1.5 text-xs whitespace-nowrap rounded-lg font-bold transition ${view === "monthly" ? "bg-blue-50 text-blue-700 border border-blue-100 shadow-sm" : "text-slate-500 hover:text-slate-900 border border-transparent"}`}>Monthly View</button>
        <button onClick={() => setView("yearly")} className={`px-4 py-1.5 text-xs whitespace-nowrap rounded-lg font-bold transition ${view === "yearly" ? "bg-blue-50 text-blue-700 border border-blue-100 shadow-sm" : "text-slate-500 hover:text-slate-900 border border-transparent"}`}>Yearly View</button>
        <button onClick={() => setView("projects")} className={`px-4 py-1.5 text-xs whitespace-nowrap rounded-lg font-bold transition ${view === "projects" ? "bg-emerald-50 text-emerald-700 border border-emerald-100 shadow-sm" : "text-slate-500 hover:text-slate-900 border border-transparent"}`}>Project Reports</button>
        <button onClick={() => setView("custom")} className={`px-4 py-1.5 text-xs whitespace-nowrap rounded-lg font-bold transition ${view === "custom" ? "bg-purple-50 text-purple-700 border border-purple-100 shadow-sm" : "text-slate-500 hover:text-slate-900 border border-transparent"}`}>Custom Export</button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
        <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition">
          <div className="flex items-center gap-3 text-slate-500 mb-3">
            <div className="p-2 rounded-lg bg-emerald-50 border border-emerald-100"><TrendingUp className="w-4 h-4 text-emerald-600" /></div>
            <span className="text-[10px] font-bold uppercase tracking-wider">True SaaS Spend</span>
          </div>
          <div className="text-3xl font-display text-slate-900 font-bold tabular-nums">
        ...
        <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition">
          <div className="flex items-center gap-3 text-slate-500 mb-3">
            <div className="p-2 rounded-lg bg-blue-50 border border-blue-100"><FolderKanban className="w-4 h-4 text-blue-600" /></div>
            <span className="text-[10px] font-bold uppercase tracking-wider">Project Allocated Cost</span>
          </div>
          <div className="text-3xl font-display text-slate-900 font-bold tabular-nums">
        ...
        <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-5 hover:shadow-md transition">
          <div className="flex items-center gap-3 text-slate-500 mb-3">
            <div className="p-2 rounded-lg bg-purple-50 border border-purple-100"><BarChart3 className="w-4 h-4 text-purple-600" /></div>
            <span className="text-[10px] font-bold uppercase tracking-wider">Purchased Licenses</span>
          </div>
          <div className="text-3xl font-display text-slate-900 font-bold tabular-nums">
      ...
      {(view === "monthly" || view === "yearly") && (
        <div className="flex items-center gap-4 mb-4 bg-white p-3 rounded-xl border border-slate-200 shadow-sm w-max">
          <div className="flex items-center gap-2">
            <Filter className="w-4 h-4 text-slate-400" />
            <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Date Filters:</span>
          </div>
          <select value={filterYear} onChange={handleYearChange} className="bg-slate-50 border border-slate-200 text-sm text-slate-900 font-bold rounded-lg px-3 py-1.5 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm cursor-pointer">
          ...
          {view === "monthly" && (
            <select 
              ...
              className={`bg-slate-50 border border-slate-200 text-sm text-slate-900 font-bold rounded-lg px-3 py-1.5 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm cursor-pointer ${filterYear === "All" ? "opacity-50 cursor-not-allowed" : ""}`}
            >
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  (view === "monthly" ? filteredMonthly : filteredYearly).map((row: any, i: number) => (
                    <tr key={`${row.period}-${i}`} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-4 font-bold text-slate-900">{row.period}</td>
                      <td className="px-5 py-4 text-center font-bold text-slate-600 tabular-nums">{row.apps}</td>
                      <td className="px-5 py-4 text-center font-bold text-slate-600 tabular-nums">{row.licenses}</td>
                      <td className="px-5 py-4 text-right text-slate-900 tabular-nums font-bold">{currency(row.saasSpend)}</td>
                    </tr>
                  ))
                )}
              </tbody>
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  data.rawProjects.map((proj: any, i: number) => {
                    return (
                      <tr key={proj._id || i} className="hover:bg-slate-50 transition-colors cursor-pointer">
                        ...
                        <td className="px-5 py-4 text-center">
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs bg-slate-100 font-bold text-slate-700">
                            {proj.memberCount || 0}
                          </span>
                        </td>
        ...
        <div className="rounded-2xl border border-slate-200 shadow-sm bg-white overflow-hidden flex flex-col">
          <div className="p-5 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4 bg-slate-50">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <Filter className="w-4 h-4 text-slate-400" />
                <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Refine Data:</span>
              </div>
              <select value={filterDept} onChange={(e) => setFilterDept(e.target.value)} className="bg-white border border-slate-200 text-sm text-slate-900 font-bold rounded-lg px-3 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm cursor-pointer">
                <option value="" disabled>Department</option>
                {uniqueDepts.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
              <select value={filterVendor} onChange={(e) => setFilterVendor(e.target.value)} className="bg-white border border-slate-200 text-sm text-slate-900 font-bold rounded-lg px-3 py-2 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm cursor-pointer">
            ...
            <button 
              onClick={downloadCSV}
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm px-4 py-2.5 rounded-xl flex items-center gap-2 transition shadow-md"
            >
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredLicenses.length === 0 ? (
                  ...
                  filteredLicenses.map((lic: any) => {
                    ...
                    return (
                      <tr key={lic._id} className="hover:bg-slate-50 transition-colors cursor-pointer">
                        ...
                        <td className="px-5 py-4 text-center">
                          <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${utilization >= 80 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : utilization < 40 ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
