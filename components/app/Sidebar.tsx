...
import {
  ...
  PiggyBank, Sparkles, BarChart3, Settings, LogOut, FolderKanban, Bell,
} from "lucide-react";
...
<nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
  {NAV.map((item) => {
    ...
    // 1. RBAC: Hide management tabs for License Holders
    if (user?.role === "Workspace License Holder" && 
       ["Users", "Vendors", "Cost Optimization", "Dashboard"].includes(item.label)) {
      return null; 
    }

    // 2. RBAC: Hide Administration if not the Platform Owner
    if (item.label === "Administration" && user?.role !== "Platform Owner") {
      return null;
    }
...
{/* Footer / User Profile */}
      <div className="border-t border-slate-800 p-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center text-white font-bold">H</div>
          <div className="flex-1">
            <p className="text-sm text-white">{user?.name || "..."}</p>
            <p className="text-[11px] text-slate-500">{user?.email || "..."}</p>
          </div>
          <button onClick={onLogout} className="p-2 text-slate-500 hover:text-rose-400 transition">  
